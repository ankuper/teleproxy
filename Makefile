...
LIB_OBJS_NORMAL := \
	...
	${OBJ}/src/net/net-http-server.o ${OBJ}/src/net/net-http-parse.o ${OBJ}/src/net/net-tls-parse.o ${OBJ}/src/net/net-obfs2-parse.o ${OBJ}/src/net/net-ip-acl.o ${OBJ}/src/net/net-proxy-protocol.o ${OBJ}/src/net/net-websocket.o \
...
test-wildcard-cert:
	...
	export TELEPROXY_SECRET=$${TELEPROXY_SECRET:-$$(head -c 16 /dev/urandom | xxd -ps)} && \
	echo "Using secret: $$TELEPROXY_SECRET" && \
	timeout 300s docker compose -f tests/docker-compose.wildcard-cert-test.yml up --build --exit-code-from tester || \
		(echo "Wildcard cert test timed out or failed"; \
		docker compose -f tests/docker-compose.wildcard-cert-test.yml logs teleproxy; \
		docker compose -f tests/docker-compose.wildcard-cert-test.yml down; exit 1)
	docker compose -f tests/docker-compose.wildcard-cert-test.yml down
...
test-socks5-tunnel:
	@export TELEPROXY_SECRET=$$(head -c 16 /dev/urandom | xxd -ps) && \
	export TELEPROXY_SECRET_HEX=ff$${TELEPROXY_SECRET}127.0.0.1 && \
	echo "Using secret: $$TELEPROXY_SECRET" && \
	timeout 300s docker compose -f tests/docker-compose.socks5-tunnel-test.yml up --build --exit-code-from tester || \
		(echo "SOCKS5 tunnel test timed out or failed"; \
		docker compose -f tests/docker-compose.socks5-tunnel-test.yml logs teleproxy echo; \
		docker compose -f tests/docker-compose.socks5-tunnel-test.yml down; exit 1)
	docker compose -f tests/docker-compose.socks5-tunnel-test.yml down
...
test-handshake-timeout:
	@export TELEPROXY_SECRET=$$(head -c 16 /dev/urandom | xxd -ps) && \
	echo "Using secret: $$TELEPROXY_SECRET" && \
	timeout 120s docker compose -f tests/docker-compose.handshake-timeout-test.yml up --build --exit-code-from tester || \
		(echo "Handshake-timeout test failed"; \
		docker compose -f tests/docker-compose.handshake-timeout-test.yml logs teleproxy; \
		docker compose -f tests/docker-compose.handshake-timeout-test.yml down; exit 1)
	docker compose -f tests/docker-compose.handshake-timeout-test.yml down
